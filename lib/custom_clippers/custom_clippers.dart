import 'package:flutter/material.dart';

// 출처: https://www.youtube.com/watch?v=P0eZ0XH5FdI&t=16s
class BigClipper extends CustomClipper<Path> {
  double? maxHeight;
  @override
  Path getClip(Size size) {
    Path path = Path();
    final double _xScaling = size.width / 414;
    final double _yScaling = size.height / 896;
    maxHeight = 413.99600000000004 * _yScaling;
    path.lineTo(
        -0.003999999999997783 * _xScaling, 291.78499999999997 * _yScaling);
    path.cubicTo(
      -0.003999999999997783 * _xScaling,
      291.78499999999997 * _yScaling,
      23.461000000000002 * _xScaling,
      313.15099999999995 * _yScaling,
      71.553 * _xScaling,
      313.15099999999995 * _yScaling,
    );
    path.cubicTo(
      119.645 * _xScaling,
      313.15099999999995 * _yScaling,
      142.21699999999998 * _xScaling,
      250.186 * _yScaling,
      203.29500000000002 * _xScaling,
      257.21 * _yScaling,
    );
    path.cubicTo(
      264.373 * _xScaling,
      264.234 * _yScaling,
      282.666 * _xScaling,
      283.47299999999996 * _yScaling,
      338.408 * _xScaling,
      283.47299999999996 * _yScaling,
    );
    path.cubicTo(
      394.15000000000003 * _xScaling,
      283.47299999999996 * _yScaling,
      413.99600000000004 * _xScaling,
      204.199 * _yScaling,
      413.99600000000004 * _xScaling,
      204.199 * _yScaling,
    );
    path.cubicTo(
      413.99600000000004 * _xScaling,
      204.199 * _yScaling,
      413.99600000000004 * _xScaling,
      0 * _yScaling,
      413.99600000000004 * _xScaling,
      0 * _yScaling,
    );
    path.cubicTo(
      413.99600000000004 * _xScaling,
      0 * _yScaling,
      -0.003999999999976467 * _xScaling,
      0 * _yScaling,
      -0.003999999999976467 * _xScaling,
      0 * _yScaling,
    );
    path.cubicTo(
      -0.003999999999976467 * _xScaling,
      0 * _yScaling,
      -0.003999999999997783 * _xScaling,
      291.78499999999997 * _yScaling,
      -0.003999999999997783 * _xScaling,
      291.78499999999997 * _yScaling,
    );
    return path;
  }

  @override
  bool shouldReclip(covariant CustomClipper<Path> oldClipper) {
    return true;
  }
}

class SmallClipper extends CustomClipper<Path> {
  @override
  Path getClip(Size size) {
    Path path = Path();
    final double _xScaling = size.width / 414;
    final double _yScaling = size.height / 896;
    path.lineTo(-0.003999999999997783 * _xScaling, 167.841 * _yScaling);
    path.cubicTo(
      -0.003999999999997783 * _xScaling,
      167.841 * _yScaling,
      19.14 * _xScaling,
      215.91999999999996 * _yScaling,
      67.233 * _xScaling,
      215.91999999999996 * _yScaling,
    );
    path.cubicTo(
      115.326 * _xScaling,
      215.91999999999996 * _yScaling,
      112.752 * _xScaling,
      184.611 * _yScaling,
      173.83299999999997 * _xScaling,
      191.635 * _yScaling,
    );
    path.cubicTo(
      234.914 * _xScaling,
      198.659 * _yScaling,
      272.866 * _xScaling,
      251.691 * _yScaling,
      328.608 * _xScaling,
      251.691 * _yScaling,
    );
    path.cubicTo(
      384.34999999999997 * _xScaling,
      251.691 * _yScaling,
      413.99600000000004 * _xScaling,
      151.977 * _yScaling,
      413.99600000000004 * _xScaling,
      151.977 * _yScaling,
    );
    path.cubicTo(
      413.99600000000004 * _xScaling,
      151.977 * _yScaling,
      413.99600000000004 * _xScaling,
      0 * _yScaling,
      413.99600000000004 * _xScaling,
      0 * _yScaling,
    );
    path.cubicTo(
      413.99600000000004 * _xScaling,
      0 * _yScaling,
      -0.003999999999976467 * _xScaling,
      0 * _yScaling,
      -0.003999999999976467 * _xScaling,
      0 * _yScaling,
    );
    path.cubicTo(
      -0.003999999999976467 * _xScaling,
      0 * _yScaling,
      -0.003999999999997783 * _xScaling,
      167.841 * _yScaling,
      -0.003999999999997783 * _xScaling,
      167.841 * _yScaling,
    );
    return path;
  }

  @override
  bool shouldReclip(covariant CustomClipper<Path> oldClipper) {
    return true;
  }
}
